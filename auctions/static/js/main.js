window.addEventListener('load', function () {
    ...
    if (overlayer) {
        overlayer.style.transition = 'opacity 1s';
    ...
    const imgElements = document.querySelectorAll('.bg_img');
    imgElements.forEach(function (img) {
        const backgroundUrl = img.dataset.background;
        if (backgroundUrl) {
            img.style.backgroundImage = 'url(' + backgroundUrl + ')';
        }
    });
});
